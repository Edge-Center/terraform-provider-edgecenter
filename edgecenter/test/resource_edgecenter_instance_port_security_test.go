...
func TestAccInstancePortSecurity(t *testing.T) {
	...
	instancePortSecurityTemplate := func(params *Params) string {
		...
		`, projectInfo(), regionInfo(), portID, instanceID, params.PortSecurityDisabled, params.Enforce, strings.Join(params.SecurityGroups, ", "))
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "security_groups.0.enforce", fmt.Sprintf("%t", create.Enforce)),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "security_groups.0.enforce", fmt.Sprintf("%t", update.Enforce)),
