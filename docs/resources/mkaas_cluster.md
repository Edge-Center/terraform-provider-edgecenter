...
- `publish_kube_api_to_internet` (Boolean) Publish kube-api to internet.
...
- `processing` (Boolean)
- `stage` (String) Stage of the Kubernetes cluster.
...
- `version` (String) The version of the Kubernetes cluster (format `vx.xx`). Validated against available versions in the region via API.
