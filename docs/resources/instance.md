...
- `volume` (Block List, Min: 1) A set defining the volumes to be attached to the instance. (see [below for nested schema](#nestedblock--volume))
- `project_id` (Number, ForceNew) The uuid of the project. Either `project_id` or `project_name` must be specified.
- `project_name` (String, ForceNew) The name of the project. Either `project_id` or `project_name` must be specified.
- `region_id` (Number, ForceNew) The uuid of the region. Either `region_id` or `region_name` must be specified.
- `region_name` (String, ForceNew) The name of the region. Either `region_id` or `region_name` must be specified.
...
- `name_template` (String) A template used to generate the instance name. You can use forms ip_octets, two_ip_octets, one_ip_octet. This field cannot be used with 'name_templates'.
...
- `user_data` (String) A string in the base64 format.A field for specifying user data to be used for configuring the instance at launch time.
...
- `existing_fip_id` (String) If fip_source is 'existing', the ID of the existing floating IP must be specified
- `fip_source` (String) Indicates whether the floating IP for this subnet will be new or reused.Possible values 'existing'.
- `network_id` (String) The ID of the network that the subnet belongs to. Required if type is 'subnet' or 'any_subnet'.
...
- `subnet_id` (String) The port is assigned an IP address from this subnet.Required if type is 'subnet'.

Read-Only:

- `ip_address` (String) Ip address which mapped to this interface.
...
- `attachment_tag` (String) The block device attachment tag.
- `boot_index` (Number, ForceNew) 0 should be set for the primary boot device. Unique positive values for other bootable devices. Negative - the boot is prohibited. If boot_index==0 volumes can not detached
...
- `volume_id` (String) The ID of the volume. The field is mandatory if the source == 'existing-volume'.
...
- `key` (String) The metadata item key. The maximum size is 255 bytes.
- `value` (String) The metadata item value. The maximum size is 1024 bytes.
...
- `id` (String) Firewall unique id (uuid).
